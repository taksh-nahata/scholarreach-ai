export class ExaClient {
  ...
  async findRecentPaper(professorName: string, university: string, researchFocus: string) {
  ...
  async searchWeb(query: string): Promise<string> {
    ...
    try {
      ...
      data.results.forEach((res, i) => {
        ...
        if (res.highlights?.length) resultString += `Highlights: ${res.highlights.join(" | ")}\n`;
        resultString += "\n";
      });
      return resultString;
    } catch (err) {
      console.warn(`[Exa] searchWeb failed:`, err instanceof Error ? err.message : err);
